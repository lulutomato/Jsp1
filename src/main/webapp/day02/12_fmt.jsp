...
<!-- formatNumber는 서식 있는 출력과 저장 동작 , value에는 값을 직접 또는 el -->
   ...
   <!--fmt: formatXXXX : 출력과 변수에 저장, fmt:parseXXXX : 변수의 저장 -->
   <hr>
   <!--화폐, 날짜는 locale 에 따라 결과가 다릅니다  -->
   ...
   <!-- parseDate의 pattern은 value 값의 형식을 알려줍니다.
   		패턴이 없으면 날짜 타입으로 변환을 못 합니다. (문자열을 날짜 타입으로 캐스팅) -->
   <fmt:parseDate value="2022-11-03 20:14:45" var="datetime1" 
               pattern="yyyy-MM-dd HH:mm:ss"/> 
    지정된 시간과 날짜 :<c:out value= "${datetime1 }"/>
    <br>  <!-- 출력결과 : Thu Nov 03 20:14:45 KST 2022 --> 		<!--아래 출력 결과는 locale en_us  -->
   날짜:<fmt:formatDate value="${datetime1 }" type="date"/><br>  <!-- Nov 3, 2022 -->
   시간:<fmt:formatDate value="${datetime1 }" type="time"/><br>  <!-- 8:14:45 PM -->
   둘다:<fmt:formatDate value="${datetime1 }" type="both"/><br>  <!-- Nov 3, 2022, 8:14:45 PM -->
